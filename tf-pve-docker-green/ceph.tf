...
output "cephfs_mon_hosts" {
  description = "From tf-pve-ceph - feed into ansible-pve-docker-green's extra-vars.yml (cephfs_mon_hosts)"
  value       = data.terraform_remote_state.ceph.outputs.mon_hosts
}

output "cephfs_name" {
  description = "From tf-pve-ceph - feed into ansible-pve-docker-green's extra-vars.yml (cephfs_name)"
