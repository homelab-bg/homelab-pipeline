...
| `/tf-pve-netbox-ipam` | `NETBOX_API_TOKEN`, `NETBOX_URL` | `tf-pve-netbox-ipam` (Terraform module managing NetBox IPAM/DCIM objects via its REST API, `e-breuninger/netbox` provider) - NetBox-side scoping: dedicated `terraform` user, write on `ipam.prefix`/`ipam.ipaddress`/`dcim.site`/`ipam.role`/`ipam.rir`/`ipam.aggregate`, view-only on `ipam.vrf`/`ipam.vlan`/`ipam.vlangroup` | `tf-pve-netbox-ipam-reader` |
| `/shared` | `MINIO_ACCESS_KEY_ID`, `MINIO_SECRET_ACCESS_KEY`, `MINIO_S3_ENDPOINT`, `GITHUB_CONFIG_REPO_TOKEN`, `PROXMOX_VE_API_TOKEN`, `ROUTE53_BAUER_ACCESS_KEY_ID`, `ROUTE53_BAUER_SECRET_ACCESS_KEY`, `ROUTE53_BAUER_REGION`, `ROUTE53_BAUER_HOSTED_ZONE_ID` | human reference; also read by every `homelab-ci` CI identity via the CLI. Only the MinIO backend creds are structurally forced into this path (see below) - `GITHUB_CONFIG_REPO_TOKEN` and `PROXMOX_VE_API_TOKEN` have no such constraint (the latter is read directly by the `proxmox` provider as a plain env var, same as any other provider credential), they're just convenient to fetch alongside the MinIO creds in one CLI call. `ROUTE53_BAUER_*` is the Route53 DNS-01 credential for the `example.com` zone, shared across every Caddy instance doing its own Let's Encrypt cert (currently `ansible-pve-netbox`'s Caddy) - NOT `ansible-pve-secrets`'s own Caddy, which keeps its own copy of the same values in plain `extra-vars.yml` for bootstrap reasons (see "Root-of-trust secrets" below) | `ci-tf-dns-technitium-reader`, `ci-tf-dns-secrets-technitium-reader`, `ci-tf-pve-docker-green-technitium-reader`, `ci-tf-pve-template-smoketest-reader`, `ci-tf-pve-packer-reader`, `ansible-pve-netbox-reader` |
